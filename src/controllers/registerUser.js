...
const registerUser = async (req, res) => {
  try {
    ...
    const organizationExist = await Organization.findOne({ where: { organizationName }});

    if (organizationExist && newUser.role === "user"){
      const registerUser = await User.create({ ...newUser, organizationId: organizationExist.id, rights: "read only"});
      return res.status(200).json({"message": "user created successfully", registerUser});
    }

    else if (!organizationExist && newUser.role === "user"){
      return res.status(404).json({"message": `${organizationName} organization not exist.`});
    }

    else if (organizationExist && newUser.role !== "user"){
      return res.status(409).json({"message": `${organizationName} organization is already booked.`});
    }

    const newOrganization = await Organization.create({ organizationName });

    if (newOrganization){
      const registerUser = await User.create({ ...newUser, organizationId: newOrganization.id });
      return res.status(200).json({"messager": "user created successfully", registerUser});
    }
  }
  catch (err){
